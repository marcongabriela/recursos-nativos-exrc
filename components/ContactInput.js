  ...
  return (
    <View style={styles.insertContact}> 
        <View style={styles.sectionTextInput}>
            ...
            <TextInput style={styles.nameTextInput} onChangeText={name => setName(name)} value={name} placeholder="Exemplo: Elis" />
        </View>
        <View style={styles.sectionTextInput}>
        ...
        <View style={{ marginBottom: 10 }} > 
          <Button color="#31f51b5" title="Adicionar contato" onPress={() => props.onAddContact(name, number) }/>
        </View>
        <View>
          <Button color="#002984" title="Limpar Contatos" onPress={() => props.onDeleteAll() }/>
        </View>
    </View>
  );
}
